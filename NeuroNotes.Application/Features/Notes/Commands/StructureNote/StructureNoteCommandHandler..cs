    ...
    public class StructureNoteCommandHandler : IRequestHandler<StructureNoteCommand>
    {
        ...
        public async Task Handle(StructureNoteCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                providerSettings["Temperature"] = _aiOptions.DefaultStructureTemperature
                ...
                _logger.LogInformation(
                    "Updating embeddings for StructuredText of Note {NoteId}.",
                    note.Id);
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Structuring failed for Note {NoteId}.",
                    note.Id);

                note.FailProcessing($"Structuring failed: {ex.Message}");
